...
/* {BILINEAR}.off */
EIF_BOOLEAN F422_2811 (EIF_REFERENCE Current)
{
	GTCX
	RTEX;
	...
	RTEAA("off", 421, Current, 0, 0, 3948);
	RTSA(Dtype(Current));
	RTSC;
	RTGC;
	RTIV(Current, RTAL);
...
/* {BILINEAR}.search */
void F422_2814 (EIF_REFERENCE Current, EIF_INTEGER_8 arg1)
{
	GTCX
	RTEX;
	EIF_BOOLEAN tb1;
	...
	RTEAA("search", 421, Current, 0, 1, 3949);
	RTSA(dtype);
	RTSC;
	RTGC;
	RTIV(Current, RTAL);
	...
	RTHOOK(3);
	(nstcall = 0, F408_2705(Current, arg1));
	if (RTAL & CK_ENSURE) {
		RTHOOK(4);
		...
		if (tb2) {
			tb1 = (EIF_BOOLEAN)(arg1 == (nstcall = 0, F644_4179(Current)));
		}
		if (tb1) {
			RTCK;
		} else {
...
void F422_1 (EIF_REFERENCE Current, int where)
{
	...
	if ((nstcall = 0, F529_3434(Current))) {
		tb1 = (nstcall = 0, F529_3435(Current));
	}
	if ((EIF_BOOLEAN) !tb1) {
		RTCK;
	} else {
