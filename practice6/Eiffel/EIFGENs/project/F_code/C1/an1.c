...
void F1_23 (EIF_REFERENCE Current)
{
	...
	RTHOOK(1);
	loc3 = RTCCL(Current);
	loc3 = RTRV(eif_new_type(620, 0x01),loc3);
	if (EIF_TEST(loc3)) {
		RTHOOK(2);
		(nstcall = 1, (FUNCTION_CAST(void, (EIF_REFERENCE)) R3032[Dtype(loc3)-620])(loc3));
	} else {
		RTHOOK(3);
		loc1 = RTLNS(eif_new_type(909, 0x01).id, 909, _OBJSIZ_1_1_0_3_0_0_0_0_);
		tr1 = (nstcall = 1, F905_6712(RTMS_EX_H("Mismatch: ",10,1538098208)));
		(nstcall = -1, F909_6994(RTCW(loc1), tr1));
		RTHOOK(4);
...
void F1_27 (EIF_REFERENCE Current, EIF_REFERENCE arg1)
{
	...
	if ((EIF_BOOLEAN)(arg1 != NULL)) {
		...
		if (EIF_TEST(loc2)) {
			...
			(nstcall = 1, F24_157(RTCW(tr1), loc2));
		} else {
			RTHOOK(5);
			loc3 = loc1;
			if ((EIF_TRUE)) {
				RTHOOK(6);
				tr1 = RTOUCR(0,(nstcall = 0, F1_24), (Current));
				(nstcall = 1, F24_155(RTCW(tr1), loc3));
			} else {
				RTHOOK(7);
				tr1 = RTOUCR(0,(nstcall = 0, F1_24), (Current));
				tr2 = (nstcall = 1, F905_6712(RTCW(loc1)));
				(nstcall = 1, F24_157(RTCW(tr1), tr2));
			}
		}
	}
	RTVI(Current, RTAL);
